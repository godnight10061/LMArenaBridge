...
class LiveModelSelectionTests(unittest.TestCase):
    def test_selects_distinct_provider_diverse_models_from_live_records(self):
        ...
        selected = _select_model_candidates(
            records, primary_model="gemini-3.5-flash", limit=5
        )
    ...
    def test_candidate_success_requirement_excludes_mandatory_gemini(self):
        ...
        successful = _successful_candidate_models(
            attempts, primary_model="gemini-3.5-flash"
        )

        self.assertEqual(
            successful, {"gpt-5.2-chat-latest", "claude-sonnet-4-6"}
        )
